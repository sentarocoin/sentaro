...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #sentarocoinTEST\r");
            Send(hSocket, "WHO #sentarocoinTEST\r");
        } else
        {
        LogPrintf("JOIN #sentarocoin\n");
            // randomly join #sentarocoin00-#sentarocoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #sentarocoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #sentarocoin%02d\r", channel_number).c_str());
        };
