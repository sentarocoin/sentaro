...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSENAmount"))
        settings.setValue("nAnonymizeSENAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeSENAmount = settings.value("nAnonymizeSENAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeSENAmount"))
        SoftSetArg("-anonymizeSENamount", settings.value("nAnonymizeSENAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case anonymizeSENAmount:
            nAnonymizeSENAmount = value.toInt();
            settings.setValue("nAnonymizeSENAmount", nAnonymizeSENAmount);
            emit anonymizeSENAmountChanged(nAnonymizeSENAmount);
            break;
